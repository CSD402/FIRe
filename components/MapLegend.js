...
function MapLegend({ data, statistic, mapViz, mapScale }) {
    ...
    useEffect(() => {
        ...
        if (mapViz === MAP_VIZS.BUBBLE) {
                ...
                legend({
                    ...
                    tickFormat: function (d, i, n) {
                        if (statisticConfig?.mapConfig?.colorScale) {
                            ...
                            return (
                                formatNumber(d, statisticConfig.format) + '+'
                            );
                        } else {
            ...
            svg.attr(
                'class',
                statisticConfig?.mapConfig?.colorScale ? 'zone' : '',
            );
        }
    }, [width, statistic, mapScale, mapViz]);
...
function legend({
    ...
    if (color.interpolate) {
        ...
        x = color
            .copy()
            .rangeRound(
                quantize(interpolate(marginLeft, width - marginRight), n),
            );
