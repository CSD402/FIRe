...
function MapVisualizer({
        ...
        if (!isDistrictView) {
            ...
            const districtData = stateCodes.reduce((res, stateCode) => {
                const districts = Object.keys(
                    ...
                    (districtName) =>
                        (districtsSet?.[stateCode] || new Set()).has(
                            districtName,
                        ) ||
                        (mapViz !== MAP_VIZS.CHOROPLETH &&
            ...
            return (features || [])
                .map((feature) => {
                    ...
                    if (!isDistrictView) {
                        ...
                        if (districtName)
                            feature.value = getMapStatistic(districtData);
                        else
    ...
    return (
        <>
            <div className='svg-parent'>
                ...
                {noDistrictData && statisticConfig?.hasPrimary && (
                    ...
                    >
                        <AlertIcon />
                        <span>
                            'District-wise data not available in state
                            bulletin',
                        </span>
                    </div>
                )}
            ...
            {mapScale && (
                <MapLegend {...{ data, statistic, mapViz, mapScale }} />
            )}
